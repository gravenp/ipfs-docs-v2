...
That problem actually exists for the internet and on your computer! Right now, content is found by location, such as…
...
Once you know where your content is (or more precisely, which peer or peers are storing each of the blocks that make up the content you’re after), you use the DHT **again** to find the current location of those peers (_routing_). So, in order to get to content, you use libp2p to query the DHT twice.
